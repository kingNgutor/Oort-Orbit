# global_loadmanager.gd
extends Node

signal progress_changed(progress)
signal load_done

var _load_screen_path = GlobalScenepath.loading_screen
...
func _process(_delta):
	...
	match load_status:
		0, 2: #int vals -> 0 = THREAD_LOAD_INVALID_RESOURCE, 2 = THREAD_LOAD_FAILED
			set_process(false)
			return
		...
		3: # THREAD_LOAD_LOADED
			...
			call_deferred("_switch_to_game") #defer function call till main thread is idle, avoiding race condition (loading screen overlay issue)
